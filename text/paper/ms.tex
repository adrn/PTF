...
Figure~\ref{fig:indices_examples} shows maximal outlier light curves for each variability statistic selected from \apwsim20,000 light curves for objects on a single CCD in PTF field 100101. %, centered at (J2000) 17:21:31.44 $+$61:07:46.6. 
...
%We apply the 1\% FPR cut to the 2097 PTF fields (\apwsim15,224 deg$^2$) with $>$10 $R$-band observations using the variability index $\eta$. Our procedure is as follows:
\begin{enumerate}
...
\tablecomments{The bolded objects are plausible microlensing candidates.}
...
%The spectrum was reduced with a script written in PyRAF, the Python-based command language for IRAF\footnote{PyRAF is a product of the Space Telescope Science Institute, which is operated by AURA for NASA.}, by S.~Douglas.  
...
% If they were ... [todo: once I re-run the fitting code on these three, some comment about the derived parameters.]
