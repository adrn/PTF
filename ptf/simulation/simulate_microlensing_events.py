...
def simulate_events_compute_indices(light_curve, num, indices):
    ...
    for event_id in range(num):
        # Draw tE from a uniform distribution from 1-1000 days
        tE = 10**np.random.uniform(0., 3.)
        pool.apply_async(_simulate_events_worker, args=(sim_light_curve, tE, reference_mag, indices), callback=callback)
        #callback(_simulate_events_worker(sim_light_curve, tE, reference_mag, indices))
